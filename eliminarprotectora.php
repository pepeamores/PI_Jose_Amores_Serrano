...
if (isset($_SESSION["esAdmin"])) {
    ...
    print "<h2>Lista de Protectoras</h2>";
    $resultado = mysqli_query($conexion, "SELECT * FROM protectoras");
    $tabla = "<table border='1' cellpadding='10'>\n";
    $tabla .= "<tr><th>Id</th><th>Nombre</th><th>Ubicación</th><th>Teléfono</th></tr>\n";
    ...
    echo $tabla;
    ?>
    <form name='formulario' action='eliminarprotectora.php' method='POST' enctype='multipart/form-data'>
    <label for="id">Introduce un ID:</label>
            <input type="text" id="id" name="id" min='1'  value='1' ><br><br>
            <button type='submit' name='enviar'>Eliminar Protectoras</button>
    </form>
<a href='admin.php'>Volver funciones</a>
    <?php
    if(isset($_POST['id'])){
        $id=$_POST['id'];
        $query="delete from `protectoras` where id='$id'";
        $query_run= mysqli_query($conexion, $query);
        if($query_run){
            echo "Protectoras eliminado";
        }else{
            echo "Protectoras no eliminado";
        }
    }
