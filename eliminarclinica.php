...
if (isset($_SESSION["esAdmin"])) {
    ...
    print "<h2>Lista Clinicas</h2>";
    $resultado = mysqli_query($conexion, "SELECT * FROM clinicas");
    ...
    echo $tabla;
    ?>
    <form name='formulario' action='eliminarclinica.php' method='POST' enctype='multipart/form-data'>
    <label for="id">Introduce un ID:</label>
            <input type="text" id="id" name="id" min='1'  value='1' ><br><br>
            <button type='submit' name='enviar'>Eliminar Clinicas</button>
    </form>
<a href='admin.php'>Volver funciones</a>
    <?php
    if(isset($_POST['id'])){
        $id=$_POST['id'];
        $query="delete from `clinicas` where id='$id'";
        $query_run= mysqli_query($conexion, $query);
        if($query_run){
            echo "Clinica eliminado";
        }else{
            echo "Clinica no eliminado";
        }
    }
