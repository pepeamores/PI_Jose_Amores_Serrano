...
<header class="header_section">
  <div class="container-fluid">
    ...
    if (isset($_SESSION["esAdmin"])) {
        ...
        $raza = recogerDatos("raza");
        $edad = recogerDatos("edad");
        $genero = recogerDatos("genero");
        $descripcion = recogerDatos("descripcion");
        $imagen = $_FILES['imagen']['tmp_name'];
        $imgContent = addslashes(file_get_contents($imagen));

        print("<h1>Se ha introducido un Perro</h1>");
        $conexion = mysqli_connect("localhost", "root", "", "animales");
        mysqli_query($conexion, "INSERT INTO perros (`raza`,`edad`, `genero`,`descripción`, `imagen`) 
                VALUES ('$raza', '$edad', '$genero', '$descripcion','$imgContent')");
